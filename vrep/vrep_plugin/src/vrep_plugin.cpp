...
// --------------------------------------------------------------------------------------
// simExtROS_setInitialRobotPose
// --------------------------------------------------------------------------------------
#define LUA_SET_INITIAL_ROBOT_POSE			"simExtROS_setInitialRobotPose"
//#define LUA_SET_INITIAL_ROBOT_POSE_TIPS 	"(double[] joints_values)=" LUA_SET_INITIAL_ROBOT_POSE "()"

//#define LUA_ENABLESUBSCRIBER_COMMAND "simExtROS_enableSubscriber"

const int inArgs_SET_INITIAL_ROBOT_POSE[]={
	1,
...
void LUA_SET_INITIAL_ROBOT_POSE_CALLBACK(SLuaCallBack* p)
{
	if (!ROS_server::isSimRosSetInitialRobotPoseData())
	{
		p->outputArgCount = 0;
	}
	else
	{
		...
		//Reset flag to deactivate callback function
	    ROS_server::resetSimRosSetInitialRobotPoseData();
	    
	    //Get the desired initial robot pose
	    vrep_common::simRosSetRobotPose::Request pose = ROS_server::getSimRosGetInitialRobotPoseMsg();
		...
		/*
		//Motor handles
		p->outputArgTypeAndSize[0] = sim_lua_arg_int | sim_lua_arg_table;	//Type = int, sim_lua_arg_table = array
		p->outputArgTypeAndSize[1] = pose.handle.size();					//Size of array
		//Set size of buffer
		p->outputInt = (simInt*) simCreateBuffer(sizeof(simInt)*pose.handle.size());
		//Store motor handles in return value
		for(size_t i = 0 ; i < pose.handle.size() ; ++i)
			p->outputInt[i] = pose.handle[i];
			
			
		//RelativetoObjectHandle
		p->outputArgTypeAndSize[2] = sim_lua_arg_int | sim_lua_arg_table;
		p->outputArgTypeAndSize[3] = pose.relativeToObjectHandle.size();
        //Set size of buffer
		p->outputInt = (simInt*) simCreateBuffer(sizeof(simInt) *pose.relativeToObjectHandle.size());
		for (size_t i = 0; i < pose.relativeToObjectHandle.size() ; ++i)
		{
			p->outputInt[i] = pose.relativeToObjectHandle[i];
		}
		*/
		
		
		//Initial Joint config (for base + lbr)
		p->outputArgTypeAndSize[0] = sim_lua_arg_float | sim_lua_arg_table; //type  
		p->outputArgTypeAndSize[1] = pose.position.size();					//Size of array
		//Set size of buffer
		p->outputFloat = (simFloat*) simCreateBuffer(sizeof(simFloat) * pose.position.size());
		for (size_t i = 0; i < pose.position.size() ; ++i)
		{
				p->outputFloat[i] = (simFloat) pose.position[i];			
			    std::cout<<pose.position[i]<<std::endl;
		}
	...
	/*
		...
		ROS_server::resetSimRosSetJointPathNewData();

		vrep_omnirob_common::simRosSetJointPath::Request::_trajectory_type t = ROS_server::getSimRosSetJointPathMsg();
		vrep_omnirob_common::simRosSetJointPath::Request::_accumulated_end_effector_distances_type d = ROS_server::getSimRosSetJointPathMsgDistances();
		std::string names = "";

		//Strings require null termination
		for (auto& it : t.joint_names)
		{
			names += it + '\0';
		}
		...
		p->outputInt[0] = t.joint_names.size();

		//names of joints
		p->outputArgTypeAndSize[2] = sim_lua_arg_string | sim_lua_arg_table;
		p->outputArgTypeAndSize[3] = t.joint_names.size();

		p->outputChar = (simChar*) simCreateBuffer(sizeof(simChar) * names.length());
		for (size_t i = 0; i < names.length(); ++i)
		{
			p->outputChar[i] = names[i];
		}

		//joints, velocity, acceleration, times
		p->outputArgTypeAndSize[4] = sim_lua_arg_float | sim_lua_arg_table;
		p->outputArgTypeAndSize[5] = t.joint_names.size() * t.points.size();
		p->outputArgTypeAndSize[6] = sim_lua_arg_float | sim_lua_arg_table;
		p->outputArgTypeAndSize[7] = d.size();

		p->outputFloat = (simFloat*) simCreateBuffer(sizeof(simFloat) * (p->outputArgTypeAndSize[5] + p->outputArgTypeAndSize[7]));

		//joints
		for (size_t i = 0; i < t.points.size(); ++i)
		{
			for (size_t j = 0; j < t.joint_names.size(); ++j)
			{
				p->outputFloat[i * t.joint_names.size() + j] = (simFloat) t.points[i].positions[j];
			}
		}
		...
		{
			p->outputFloat[(t.joint_names.size() * t.points.size()) + j] = d[j];
		}
		*/
	}
...
VREP_DLLEXPORT unsigned char v_repStart(void* reservedPointer,int reservedInt)
{
	...
	CLuaFunctionData::getInputDataForFunctionRegistration(inArgs_SET_INITIAL_ROBOT_POSE,inArgs);
	simRegisterCustomLuaFunction(LUA_SET_INITIAL_ROBOT_POSE,strConCat("double[] joint_values=",LUA_SET_INITIAL_ROBOT_POSE," "),&inArgs[0],LUA_SET_INITIAL_ROBOT_POSE_CALLBACK);
