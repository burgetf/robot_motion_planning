...
//--------------- Testing 
ros::ServiceServer ROS_server::simRosSetInitialRobotPoseServer;

//Variable that stores the incoming data
vrep_common::simRosSetRobotPose::Request ROS_server::initialRobotPoseMsg;

//Flag triggering the callback in vrep_plugin.cpp
bool ROS_server::simRosSetInitialRobotPoseData = false;
...
//	simRosGetObjectPositionServer = node->advertiseService("simRosGetObjectPosition",ROS_server::simRosGetObjectPositionService);
	...
	//------ Testing --------
	simRosSetInitialRobotPoseServer = node->advertiseService("simRosSetInitialRobotPose",ROS_server::simRosSetInitialRobotPoseService);
	//-----------------------
...
bool ROS_server::isSimRosSetInitialRobotPoseData()
{
	return simRosSetInitialRobotPoseData;
}

void ROS_server::resetSimRosSetInitialRobotPoseData()
{
	simRosSetInitialRobotPoseData = false;
}


const vrep_common::simRosSetRobotPose::Request& ROS_server::getSimRosGetInitialRobotPoseMsg()
{
	return initialRobotPoseMsg;
}



//Implement Get-Functions for motor handles and positions (base configuration and LBR configuration)


//Service to store parameter and to activate setinitialRobotPose callback function 
bool ROS_server::simRosSetInitialRobotPoseService(vrep_common::simRosSetRobotPose::Request &req,vrep_common::simRosSetRobotPose::Response &res)
{
	//simSetThreadAutomaticSwitch(true);
	int errorModeSaved=_handleServiceErrors_start();
	
	ROS_INFO("Received %d initial robot config parameter", (int)req.position.size());
	
	//Store motor handles and position and orientation values here in a class variable
	initialRobotPoseMsg = req;
	
	//for(int i = 0 ; i < initialRobotPoseMsg.position.size() ; i++)
	//	std::cout<<initialRobotPoseMsg.position[i]<<std::endl;
	
	//Set flag to true (thus activating function callback in vrep_plugin.cpp)
	simRosSetInitialRobotPoseData = true;
