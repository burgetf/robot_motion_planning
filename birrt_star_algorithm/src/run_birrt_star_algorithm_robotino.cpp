/*
 *run_birrt_star_algorithm_robotino.cpp
 *
...
{
    //Init Node
    ros::init(argc, argv, "birrt_star_algorithm_robotino_node");
    ...
    string planning_group = "robotino_robot";
    ...
    //moveit_msgs::AttachedCollisionObject cart = world_builder.insertManipulableCart("cart", obj_pos, obs_dim);
...
//    constraint_vec_goal_pose[5] = 1; //RotZ
    ...
    start_ee_pose[0] = obj_pos[0];//3.6; //X
    start_ee_pose[1] = obj_pos[1] + 0.5;  //Y
    start_ee_pose[2] = 0.6;//obj_pos[2];  //Z
    ...
    //Attach the object before planning
    ...
    permitted_coordinate_dev[5].first = -0.52;    //negative Zrot deviation
    permitted_coordinate_dev[5].second = 0.52;   //positive Zrot deviation
    //planner.setParameterizedTaskFrame(constraint_vector, permitted_coordinate_dev, true, true);
    //planner.setParameterizedTaskFrame(constraint_vector, permitted_coordinate_dev, bool task_pos_global, bool task_orient_global);
    ...
    vector<double> edge_cost_weights(8);
    ...
    planner.run_planner(SEARCH_SPACE, FLAG_ITERATIONS_OR_TIME, MAX_ITERATIONS_TIME, RVIZ_SHOW_TREE, 0.0, planner_run_number);
