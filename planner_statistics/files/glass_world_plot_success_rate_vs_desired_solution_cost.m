...
for i = 1:num_planners
    ...
    if(index_lowest_cost_reached>0 && desired_solution_path_costs(index_lowest_cost_reached) < overall_lowest_cost_reached)       
...
%set(h, 'Position', [0.0 0.0 1000.0 1000.0])
...
%hL = gridLegend(hdl,3,planner_legend_names,'location','southoutside','Orientation','Horizontal','Fontsize',8);

% Get current position (which I used to keep overall size the same)
%currentLegendPosition = hL.Position;
% Define new position
%newLegendPosition = [currentLegendPosition(1) -0.0 currentLegendPosition([3 4])];
% Set new position
...
%title('Desired Final Solution Cost vs. Success Rate','FontSize',17);
xlabel('Desired Solution Cost $c_{SP}$','Interpreter','LaTex','FontSize',17)
ylabel('Success rate [\%]','Interpreter','LaTex','FontSize',17)
title('Desired Final Solution Cost vs. Success Rate','Interpreter','LaTex','FontSize',17)
