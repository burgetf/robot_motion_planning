...
for i = 1:num_planners
    ...
    for j = 0:run_indices
        ...
        if planner_statistics{2}{3} == 1
            ...
            while (last_solution_time <= post_solution_time)
                last_solution_time = last_solution_time + 0.4;
    ...
    if(0 < successful_run(i))
        ...
        if(1 < successful_run(i))
            ...
            %Only one successful run, thus standard deviation remains zero
        end
    end
...
%l = legend(planner_legend_names{1}, planner_legend_names{2}, planner_legend_names{3},planner_legend_names{4},planner_legend_names{5},planner_legend_names{6},planner_legend_names{7},planner_legend_names{8},'Hypothetical Optimal Cost','Location','southoutside');
%set(l, 'Interpreter', 'none')
%hL = gridLegend(hdl,3,planner_legend_names,'Orientation','Horizontal','Fontsize',8);
